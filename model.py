...
import feat_gen
import pdb
from parameters import opt_model
pad_value = 0
cfg_params = cfgparams.CfgParams().load()
logger = data_utils.get_logger("Model", cfg_params.log_fn)

# %%
def train_model(x_trn, y_trn,target):
    #model = RandomForestRegressor()
    if target=="Temperature":
      logger.info(f"Get the optimized model for {target}")
...
# %%
def get_trained_model(x_trn, y_trn,target):
    ...
    if len(y_trn) > 1 and y_trn.shape[1] == 1:
        model = train_model(x_trn, y_trn,target)
    else:
...
# %%
def get_model(cfg_params, args, target, dftrn):
    if args.mode == 'train':
        x_trn = get_feats(cfg_params, dftrn)
        y_trn = dftrn[target]
        logger.info(f"{target} train data - x: {x_trn.shape}, y: {y_trn.shape}")
        model = get_trained_model(x_trn, y_trn,target)
        model['train_middle'] = get_train_middle(cfg_params, target, y_trn)
    else:
        with open(Path(cfg_params.experiment_path)/(cfg_params.experiment_name+"_models_info.pkl"), 'rb') as f:
            models_info = pickle.load(f)
            model = models_info[target]
    return model


# %%
def run(cfg_params, args):
    np.random.seed(0)
    ...
    for i,target in enumerate(targets):
        ...
        if target not in dftrn.columns:
            ...
            continue
        models_info[target] = get_model(cfg_params, args, target, dftrn)
        evals_info[target] = get_eval_info(cfg_params, models_info[target], target, dftrn, dftes)
        
    if args.mode == 'train':
        with open(Path(cfg_params.experiment_path)/(cfg_params.experiment_name+"_models_info.pkl"), 'wb') as f:
...
if __name__ == '__main__':
    cfg_params = cfgparams.CfgParams().load()
    logger = data_utils.get_logger("Model Training", cfg_params.log_fn)
    parser = argparse.ArgumentParser()
    parser.add_argument("-m", "--mode", help="Specify whether to train or eval", choices=["train", "eval"], default="train")
    #default = ["--mode", "train"]
    #default = []
